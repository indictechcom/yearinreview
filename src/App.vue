  ...
  <div>
    <page v-if="currentPage <= 0 && !activePage">
      ...
      <div v-if="loading > 0">
        ...
        <div>
          Retrieving data for <span>{{ status }}</span> {{ previousYear }}.
        </div>
      </div>
      <cdx-message v-if="error" type="error"
        >An error occurred while trying to check that. Did you use the correct
        username?</cdx-message
      >
      <footer>
        Made lovingly by <a href="https://jdlrobson.com">Jon Robson</a>. This
        tool is a personal project and not affiliated or sponsored by the
        Wikimedia Foundation.
      </footer>
      <div class="license">
        All illustrations CC0 1.0 adapted from
        <a
          href="https://commons.wikimedia.org/wiki/Category:Adapted_Wikipedia_20"
          >Jasmina El Bouamraoui and Karabo Poppy Moletsane for Wikipedia 20</a
        >
        unless stated.
      </div>
    ...
    <page v-if="pages.length && currentPage >= pages.length">
      ...
      <div class="sharebox" id="statBox">
        ...
        <img
          src="https://upload.wikimedia.org/wikipedia/commons/a/a7/Adapted_Wikipedia20symbol_puzzleglobe.svg"
          width="512"
          height="401"
        />
        <div class="stats">
        ...
        <footer>
          Generate your own Year in Review at
      ...
      <p>
        Thank you for viewing your year in review and for contributing to the
        sum of all human knowledge.
      </p>
      <p class="happy">Happy {{ nextYear }}!</p>
      <cdx-message v-if="feedback" type="success">{{ feedback }}</cdx-message>
      <cdx-message v-if="error" type="error"
        >An error occurred while trying to share.</cdx-message
      >
      <cdx-button
        v-if="shareable"
        @click="shareIt"
        ...
        try {
          ...
          try {
            navigator.clipboard.writeText(
              `${SHARE_TEXT} Edits: ${this.editCount}, Discussions: ${this.talkCount}, Thanks: ${this.thanksCount}, Thanked: ${this.thankedCount} #wikipediaYIR`
            );
